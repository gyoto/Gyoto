...
\item[gyoto.python] exposes the derived classes compiled in the Python
  plug-in, \texttt{libgyoto-python3}, that allow implementing new
  Gyoto classes direclty in Python;
\item[gyoto.lorene] exposes the derived classes compiled in the Lorene
  plug-in, \texttt{libgyoto-lorene}, that allow implementing numerical
...
\item[gyoto.animate] contains a frame-work for producing videos based
  on Gyoto;
\item[gyoto.<some plug-in>] can be generated on the fly to expose the
...
contain several examples and test cases. The Python source for
gyoto.util and gyoto.animate also provide rich examples, always more
up-to-date than the present documentation.
