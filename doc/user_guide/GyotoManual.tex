% Copyright 2014-2025 Frédéric Vincent, Thibaut Paumard
%
...
\Large{Updated December 06, 2025}
%\today
...
\end{code}  
which will create a \texttt{Gyoto} repository. It contains directories \texttt{bin}, \texttt{lib}, \texttt{include}, \texttt{doc}, \texttt{python} and \texttt{plugins} containing respectively the core code and executable, the \texttt{.C} source files, the \texttt{.h} headers, the documentation, the Python bindings and plug-ins including the Python plug-in.
...
\begin{code}
 $ ipython3 -m gyoto.gtk4.apps.gyotoy
\end{code}
%$
...
\begin{itemize}
\item Exposing \texttt{libgoyto*}:
  \begin{description}
  \item[gyoto.core] exposes the generic framework compiled in
    \texttt{libgyoto};
  \item[gyoto.std] exposes the derived classes compiled in the
    standard plug-in, \texttt{libgyoto-stdplug};
  \item[gyoto.python] exposes the derived classes compiled in the
    Python plug-in, \texttt{libgyoto-python3}, that allow implementing
    new Gyoto classes direclty in Python;
  \item[gyoto.lorene] exposes the derived classes compiled in the
    Lorene plug-in, \texttt{libgyoto-lorene}, that allow implementing
    numerical Metrics based on LORENE;
  \item[gyoto.<some plug-in>] can be generated on the fly to expose
    the classes implemented in any new custom plug-in;
  \item[gyoto.metric, gyoto.astrobj, gyoto.spectrometer,
    gyoto.spectrum] regroup the various Metrics, Astrobjs, etc. from
    the various extensions.
  \end{description}
\item Pure Python helpers and high-level routines:
  \begin{description}
  \item[gyoto.animate] contains a frame-work for producing videos based
    on Gyoto;
  \item[gyoto.gtk4] contains GTK4 graphical user interfaces (notably
    gyotoy() and edit()) and widgets for building such custom
    applications;
  \item[gyoto.matte\_painting] provides functionality to visualize
    gravitational lensing effects on painted backgrounds;
  \item[gyoto.utils] contains a few high-level wrappers and helper
    functions.
  \end{description}
\end{itemize}
...
Sample code can be found in \texttt{python/example*.py},
\texttt{python/tests/*.py} and \\
\texttt{plugins/python/doc/examples}. The
...
Importing \texttt{gyoto} should be sufficient as it loads most of the
rest automatically. Helper functions and syntactic sugar are provided
...
\begin{code}
  sc=gyoto.utils.readScenery("../doc/examples/example-moving-star.xml")
\end{code}

Print an XML representation of an object:
\begin{code}
  print(sc)
\end{code}
...
Ray-trace a scenery:
\begin{code}
  data = sc[:,:] # data is a dict containing the various requested quantities.
\end{code}

Create Astrobj by name:
\begin{code}
  tr=gyoto.astrobj.Torus()
\end{code}
Most Gyoto objects inherit from \texttt{Gyoto::Object} and have
properties that can be accessed as Python attributes:
\begin{code}
  tr.SmallRadius = 0.5
\end{code}
to get a list of the properties that can be set in an object:
\begin{code}
  tr.help()
\end{code}

A lot of properties accept units. Note that a Metric must be set for
physical distances to work in a lot of cases as the mass of the Metric
sets the scale between meters and geometrical units.
\begin{code}
  sc.Metric.Mass = 4e6, "sunmass"
  sc.Astrobj.Radius = 2e7, "km"
\end{code}
To retrieve a property in a given unit, it is necessary to used the
\texttt{get()} method.
\begin{code}
  sc.Astrobj.get("Radius", "km")
\end{code}

Methods of derived classes in the standard and lorene plug-ins (for
instance the Star API, which allows computing time-like geodesics) are
exposed respectively in the \texttt{gyoto.std} and
\texttt{gyoto.lorene} submodules:
\begin{code}
  st=gyoto.std.Star()
\end{code}
...
\begin{code}
  ...
  st = gyoto.std.Star(ao) # if conversion fails, error is thrown
\end{code}

It is very easy to implement new Metric and Astrobj kinds in Python
using the \texttt{gyoto.python} module, which wraps the python
plug-in (Sect.~\ref{sect:python-plug-in}).

It is also possible to extend Gyoto with a custom C++ plug-in
(Sect.~\ref{sect:plug-in}). Such plug-ins can easily be accessed from
Python. In the following, we will use the \texttt{stdplug} plug-in as
an example, but it would work the same wih any plug-in as long as the
name of the plug-in and of the classes that it registers are valid
Python identifiers:
\begin{code}
  import gyoto.stdplug
  st = gyoto.stdplug.Star() # st containts a Gyoto::Astrobj::Generic* pointer
  st.Metric = gyoto.metric.KerrBL()
  st.Metric.Mass = 4e6, "sunmass"
  st.Radius = 2e7, "km"
  # or, in one go:
  st = gyoto.stdplug.Star(
...
The source directories \texttt{python} and \texttt{plugins/python}
contain several examples and test cases. The Python source for gyoto
submodules also provide rich examples, always more up-to-date than the
present documentation.

\subsection{Using the Yorick plug-in}
\label{sect:yorick}
Warning: the Yorick plug-in is deprecated and will be rempoved
soon. Use Python for new projects and port your Gyoto-based Yorick
projects to Python.
...
\begin{code}
  ...
  metric = gyoto.metric.KerrBL()        # make and initialize a metric

  scr = gyoto.core.Screen()             # make a Screen
  scr.Metric       = metric
  scr.Resolution   = 32
  scr.FieldOfView  = 60., 'degree'
  scr.AngleKind    = 'Rectilinear'

  ao = gyoto.astrobj.ThinDisk()         # make an astrobj,
  ao.Metric        = metric             # initialize it
  ao.InnerRadius   = 6.
  ao.RMax          = 200.

  sc = gyoto.core.Scenery()             # make Scenery
...
\begin{code}
  plt.imshow(results['Intensity'], origin='lower')
  plt.show()
\end{code}
...
\texttt{help(gyoto.metric.Generic.circularVelocity)}), but to continue with the
same example we shall retrieve the velocity of a ZAMO with:

\begin{code}
  fourvel = metric.zamoVelocity(pos)
\end{code}
...
\begin{code}
  pos=numpy.asarray([x0, x1, x2, x3]) # this is a 4 position
  fourvel = metric.circularVelocity(pos)
...
\item each subcontractor registered by the init function (see below)
  ...
  search for a registered subcontractor by kind name and plug-in name.
\end{itemize}
...
the standard plug-in) in a plug-in called \texttt{myplugin}, you would
...
\end{code}
We have seen already that a custom plug-in is very easy to access from
Python:
\begin{code}
  import gyoto.stdplug
  metric = gyoto.stdplug.KerrBL()
\end{code}
