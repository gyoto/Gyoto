...
\begin{code}
  ...
  import numpy
  metric=core.Metric('MetricKind')    # take a metric
  scr=core.Screen()                   # make a Screen
  scr.metric(metric)                  
  ao=core.Astrobj('AstobjKind')       # make astrobj
  ao.metric(metric)
  ...
  scr.setObservPos(pos)               # set observer position
  fourvel=numpy.zeros(4)              # NumPy array for 4-velocity
\end{code}

Now we can set the observer kind. The default would be:
\begin{code}
  scr.observerKind('ObserverAtInfinity')
\end{code}
which will not work close to the central object.

\begin{code}
  scr.observerKind('ZAMO')
\end{code}
would work, though. Nothing else would be required and one could then proceed with ray-tracing:

\begin{code}
  results=util.rayTrace(sc)
\end{code}

To try \texttt{ObserverKind==VelocitySpecified}, we need to get a
4-velocity. Several methods can help with that, but in the general
case it's up to the user:

\begin{code}
  metric.circularVelocity(pos, fourvel)
\end{code}

\texttt{circularVelocity} is defined by the Metrics to provide the
velocity that a particle must have, at the position \texttt{pos}
projected onto the equatorial plane, to be on a circular orbit. Such a
velocity does not exist everywhere in the general case and only some
Metrics provide this method. The Keplerian approximation is defined in
the general case by:

\begin{code}
  metric.keplerian(True)
  metric.circularVelocity(pos, fourvel)
\end{code}

but even in that case, the 4-velocity thus constructed does not always
make sense.
...
zero angular momentum (ZAMO stands for `zero angular momentum
observer'). \texttt{KerrBL.zamoVelocity} is an optimised
...
/ \dot x^0)$. $\vec e_u$ is initialized as $\proj_u(\vec e_r)$ and
...
e_l, \vec e_u, \vec e_f)$ in the metric sense. Most computations are
...
\begin{code}
  ...
  # sense:
  metric.GramSchmidt(pos, vel, eu, ef, el);
