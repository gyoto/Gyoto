...
GYOTO_PROPERTY_START(Sim2DThinDisk, "Equatorial thin disk painted with exterior intensity")
GYOTO_PROPERTY_DOUBLE_UNIT(Sim2DThinDisk, InnerRadius, innerRadius, "Inner radius (geometrical units, 0).")
GYOTO_PROPERTY_DOUBLE_UNIT(Sim2DThinDisk, OuterRadius, outerRadius, "Outer radius (geometrical units, DBL_MAX).")
GYOTO_PROPERTY_DOUBLE_UNIT(Sim2DThinDisk, Thickness, thickness, "Geometrical thickness (geometrical units, 1e-3, for optical depth).")
GYOTO_PROPERTY_END(Sim2DThinDisk, SimBridge::properties)

Sim2DThinDisk::Sim2DThinDisk() : 
  Astrobj::SimBridge(), 
  rin_(0.), 
  rout_(DBL_MAX), 
  thickness_(1e-3)
...
Sim2DThinDisk::Sim2DThinDisk(const Sim2DThinDisk& orig) : 
...
Sim2DThinDisk* Sim2DThinDisk::clone() const { return new Sim2DThinDisk(*this); }

Sim2DThinDisk::~Sim2DThinDisk() {
  if (debug()) cerr << "DEBUG: Sim2DThinDisk::~Sim2DThinDisk()\n";
}

string Sim2DThinDisk::className() const { return  string("Sim2DThinDisk"); }
string Sim2DThinDisk::className_l() const { return  string("sim2dThindisk"); }


double Sim2DThinDisk::innerRadius() const   { return rin_; }
double Sim2DThinDisk::innerRadius(string const &unit) const   {
  return Units::FromGeometrical(innerRadius(), unit, gg_);
}
void   Sim2DThinDisk::innerRadius(double r) { rin_ = r;    }
void   Sim2DThinDisk::innerRadius(double r, string const &unit) {
  innerRadius(Units::ToGeometrical(r, unit, gg_));
}


double Sim2DThinDisk::outerRadius() const   { return rout_;}
double Sim2DThinDisk::outerRadius(string const &unit) const   {
  return Units::FromGeometrical(outerRadius(), unit, gg_);
}
void   Sim2DThinDisk::outerRadius(double r) { rout_ = r;   }
void   Sim2DThinDisk::outerRadius(double r, string const &unit) {
  outerRadius(Units::ToGeometrical(r, unit, gg_));
}


double Sim2DThinDisk::thickness() const     { return thickness_;}
double Sim2DThinDisk::thickness(string const &unit) const   {
  return Units::FromGeometrical(thickness(), unit, gg_);
}
void   Sim2DThinDisk::thickness(double h)   { thickness_ = h;   }
void   Sim2DThinDisk::thickness(double h, string const &unit)   {
  thickness(Units::ToGeometrical(h, unit, gg_));
}


double Sim2DThinDisk::operator()(double const coord[4]) {
  double theta;
  ...
  case GYOTO_COORDKIND_CARTESIAN:
    return coord[3];
  default:
    GYOTO_ERROR("Sim2DThinDisk::Impact(): unknown COORDKIND");
    return 0.;
  }
}


double Sim2DThinDisk::projectedRadius(double const coord[4]) const {
  switch (gg_ -> coordKind()) {
  case GYOTO_COORDKIND_SPHERICAL:
    return coord[1];
  case GYOTO_COORDKIND_CARTESIAN:
...
int Sim2DThinDisk::Impact(Photon *ph, size_t index,
